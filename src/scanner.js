...
function rssItems(xml, source) {
  ...
  return items.slice(0, 60).map((raw, rank) => {
    ...
    const traffic = Number(trafficRaw.replace(/[^0-9]/g,'')) || 0;
    return {title, source, rank: rank + 1, traffic};
...
function keyOf(s) {
  return s.toLowerCase().replace(/[^a-z0-9 ]/g,' ').replace(/\b(the|a|an|and|or|of|to|in|on|for|with|vs|at|by)\b/g,' ').replace(/\s+/g,' ').trim().slice(0,100);
}
function words(s) { return keyOf(s).split(' ').filter(w => w.length > 2); }
function containsBlocked(s) {
  const t = s.toLowerCase();
  return CONFIG.blockedNarrativeTerms.some(term => t.includes(term));
}
function category(s) {
  const t=s.toLowerCase();
  if (/game|gaming|xbox|playstation|steam|nintendo/.test(t)) return 'gaming';
  if (/ai|robot|tech|iphone|android|software|space/.test(t)) return 'technology';
  if (/football|soccer|nba|nfl|f1|tennis|goal|match/.test(t)) return 'sports';
  if (/crypto|bitcoin|ethereum|solana|token|defi/.test(t)) return 'crypto';
  if (/meme|viral|internet|trend/.test(t)) return 'internet-culture';
  return 'general-culture';
}
function tickerFrom(title) {
  const banned = new Set(['THE','AND','FOR','WITH','FROM','THIS','THAT','NEWS','LIVE','TODAY']);
  const ws = words(title).map(x=>x.toUpperCase()).filter(x=>!banned.has(x));
  return (ws[0] || 'PULSE').replace(/[^A-Z0-9]/g,'').slice(0,8) || 'PULSE';
}

async function dexSaturation(title) {
  const q = encodeURIComponent(words(title).slice(0,4).join(' '));
  if (!q) return {pairs:0, active:0, liquid:0};
  try {
    const j = await getJson(`https://api.dexscreener.com/latest/dex/search?q=${q}`);
    const pairs = Array.isArray(j?.pairs) ? j.pairs : [];
    const active = pairs.filter(p => Number(p?.volume?.h24||0) > 10000).length;
    const liquid = pairs.filter(p => Number(p?.liquidity?.usd||0) > 10000).length;
    return {pairs:pairs.length, active, liquid};
...
async function main() {
  ...
  try {
    const j=await getJson(CONFIG.feeds.coinGeckoTrending);
    for (const c of (j?.coins||[])) {
      const item=c?.item||{};
      for (const n of [item.name,item.symbol]) if (n) cryptoNames.add(String(n).toLowerCase());
    }
  } catch(e) { errors.push(`coingecko:${e.message}`); }

  const grouped=new Map();
  for (const item of raw) {
    if (containsBlocked(item.title)) continue;
    const k=keyOf(item.title); if (!k) continue;
    const existing=grouped.get(k)||{title:item.title, mentions:[], bestRank:999, traffic:0};
    existing.mentions.push(item.source); existing.bestRank=Math.min(existing.bestRank,item.rank); existing.traffic=Math.max(existing.traffic,item.traffic||0);
    grouped.set(k,existing);
  }

  let candidates=[...grouped.values()].map(x=>{
    const uniq=[...new Set(x.mentions)];
    let score=42;
    if (uniq.includes('google-trends')) score+=24;
    if (uniq.includes('google-news')) score+=8;
  ...
  let previous={runs:[]};
  try { previous=JSON.parse(await fs.readFile(HISTORY,'utf8')); } catch {}
  ...
  await fs.writeFile(HISTORY,JSON.stringify(previous,null,2));
  console.log(JSON.stringify({generatedAt:now,qualifiedCount:qualified.length,top:qualified.slice(0,5).map(x=>({title:x.title,score:x.score}))},null,2));
}
