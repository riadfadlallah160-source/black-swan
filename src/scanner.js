...
const UA = 'BlackSwanNarrativeScanner/1.2';
...
function category(s){
  ...
  if(hasAny(t,['meme','viral','internet','trend']))return 'internet-culture';
...
function finishScore(c){
  ...
  if(c.dex.active>=8)c.score-=30;else if(c.dex.active>=3)c.score-=18;else if(c.dex.pairs>=12)c.score-=12;if(c.dex.pairs===0)c.score+=6;
...
async function main(){
  ...
  let candidates=[...grouped.values()].map(x=>{const uniq=[...new Set(x.mentions)];let score=42;if(uniq.includes('google-trends'))score+=24;if(uniq.includes('google-news'))score+=8;if(uniq.length>1)score+=10;score+=Math.max(0,12-Math.floor(x.bestRank/5));if(x.traffic>=100000)score+=8;else if(x.traffic>=20000)score+=5;const overlap=[...cryptoNames].some(n=>n.length>3&&x.title.toLowerCase().includes(n));if(overlap)score-=18;return{...x,sources:uniq,score,category:category(x.title),cryptoTrendingOverlap:overlap};}).sort((a,b)=>b.score-a.score).slice(0,24);
